...
bool carPooling(vector<vector<int>> &trips, int capacity) {
  ...
  for (int i = 0; i < trips.size(); i++) {
    // storing the source and number of people who will enter on this source
    v.push_back({trips[i][1], trips[i][0]});
    // storing the destination and number of people who will exit 
    // that's why '-' symbol, it means these number of people will go out of car.
  ...
  // now we will check if the car is full or not at each point and 
  // filled is the number of people in the car at each point
  int filled = 0;
