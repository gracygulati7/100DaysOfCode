//! GeeksForGeeks Question Count BST nodes that lie in a given range
...
int getCount(Node *root, int l, int h)
{
    ...
    // If the root data >= l and root data <= h, means the root data lies in the range
    if (root->data >= l && root->data <= h)
    {
        ...
        // together to find the total number of nodes within the given range in the Binary Search Tree.
        return 1 + getCount(root->left, l, h) + getCount(root->right, l, h);
        // If the root data is less than the lower limit means the root data is less than the range
    }
    else if (root->data < l)
    {
